...
/**
	...
	function rubesreview_frontend_form_register() {
		...
		$cmb->add_field( array(
			'name'    => __( 'New Post Title', 'rubesreview-evaluations' ),
		...
		$cmb->add_field( array(
			'default_cb' => 'rubesreview_maybe_set_default_from_posted_values',
			'name'       => __( 'Featured Image for New Post', 'rubesreview-evaluations' ),
		...
		$cmb->add_field( array(
			'default_cb' => 'rubesreview_maybe_set_default_from_posted_values',
			'name'       => __( 'Categories', 'rubesreview-evaluations' ),
		...
		$cmb->add_field( array(
			...
			'desc'       => __( 'Please enter your name for author credit on the new post.', 'rubesreview-evaluations' ),
		...
		$cmb->add_field( array(
			...
			'desc'       => __( 'Please enter your email so we can contact you if we use your post.', 'rubesreview-evaluations' ),
	...
	function rubesreview_do_frontend_form_submission_shortcode( $atts = array() ) {
		...
		if ( isset( $_GET['post_submitted'] ) && ( $post = get_post( absint( $_GET['post_submitted'] ) ) ) ) {
			...
			$output .= '<h3>' . sprintf( __( 'Thank you%s, your new post has been submitted and is pending review by a site administrator.', 'rubesreview-evaluations' ), esc_html( $name ) ) . '</h3>';
		}
	
		// Get our form
		$output .= cmb2_get_metabox_form( $cmb, 'fake-oject-id', array( 'save_button' => __( 'Submit Post', 'rubesreview-evaluations' ) ) );
	
		return $output;
	}
	// add_shortcode( 'cmb_frontend_form', 'rubesreview_do_frontend_form_submission_shortcode' );
